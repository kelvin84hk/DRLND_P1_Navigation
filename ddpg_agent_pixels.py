...
from model_pixels import Actor,Critic,CriticD4PG
        ...
        if self.IsPR:
            ...
            if len(self.rewards_queue)>=self.N_step: # N-steps return: r= r1+gamma*r2+..+gamma^(t-1)*rt  
                ...
                if self.IsD4PG_Cat:
                    ...
                    self.critic_local.train()   
                    action_next = self.actor_target(next_state)
                    Q_target_next = self.critic_target(next_state, action_next).cpu().data    
    ...
    def distr_projection(self,next_distr_v, rewards_v, dones_mask_t, gamma):
        ...
        if dones_mask.any():
            ...
            if dones_mask.shape==():
                ...
                if eq_dones.any():
                    try:
                        proj_distr[np.array(eq_dones), np.array(l)] = 1.0
                    except:
                        pass
                        #print(eq_dones)
                        #print(l)    
                ...
                if ne_dones.any():
                    try:
                        proj_distr[np.array(ne_dones), np.array(l)] = (u - b_j)[ne_mask]
                    except:
                        #print(l)
                        pass
                    try:    
                        proj_distr[np.array(ne_dones), np.array(u)] = (b_j - l)[ne_mask]
                    except:
                        #print(u)
                        pass
